...
- **Сохранение префиксов и правил**  
  ...
  — Учитываются правила `@counter-style` и ряд других критичных at-rule-блоков.  
...
1. **Ручной QA-обход страниц**
   ...
   * Для каждой страницы переключается режим отображения (десктоп, планшет, мобильный) через DevTools, чтобы учесть все медиазапросы и адаптивные стили.
   ...
   Подробнее: [QA Чек-лист для обнаружения слепых зон CSS анализатора](#qa-чек-лист-для-обнаружения-слепых-зон-css-анализатора)
...
#### 1. Ат‑правила и структуры CSS

Управляют сохранением глобальных блоков CSS.

* **@media запросы**
  Защищает все медиа‑правила (`@media {…}`), включая вложенные селекторы. Гарантирует корректную адаптивность на разных устройствах и разрешениях.
...
* **@media print запросы**
  Сохраняет стили для печати (`@media print {…}`), которые редко тестируются в обычном режиме.
...
* **@supports проверки**
  Оставляет блоки проверки возможностей браузера (`@supports {…}`) для fallback‑стилей и кроссбраузерной совместимости.
...
* **@keyframes анимации**
  Сохраняет все ключевые кадры (`@keyframes`) и связанные свойства `animation-*`, чтобы анимации работали при любых условиях.
...
* **@font-face шрифты**
  Никогда не удаляет подключения шрифтов через `@font-face`, чтобы избежать проблем с типографикой при асинхронной загрузке.
...
* **@page правила**
  Сохраняет блоки `@page` для форматирования страниц при печати.
...
* **@counter-style счётчики**
  Защищает пользовательские схемы нумерации (`@counter-style`), которые могут использоваться в разных контекстах.
...
* **@layer слои**
  Оставляет правила CSS Cascade Layers (`@layer`) для правильного порядка применения стилей.
...
#### 2. Селекторы и их вариации

Определяют, какие селекторы оставить по-разному сложным критериям.

* **Псевдо‑классы** (`:hover`, `:focus`, `:active` и др.)
  Защищает интерактивные состояния, которые не всегда попадают в автоматический анализ.
...
* **Псевдо‑элементы** (`::before`, `::after`, `::marker` и др.)
  Оставляет генерируемый контент, важный для декоративных и структурных целей.
...
* **Атрибутные селекторы** (`[attr=value]`, `[data-*]` и др.)
  Сохраняет селекторы по атрибутам, часто используемые в динамическом контенте и JS‑компонентах.
...
* **nth‑селекторы** (`:nth-child()`, `:nth-of-type()`)
  Оставляет сложные выражения, зависящие от структуры DOM.
...
* **Логические селекторы** (`:not()`, `:is()`, `:has()` и др.)
  Сохраняет современные конструкции для сложных условий.
...
* **Соседние селекторы** (`E + F`)
  Защищает правила «adjacent sibling», зависящие от порядка элементов.
...
* **Дочерние селекторы** (`E > F`)
  Оставляет прямые отношения «parent > child», важные для вложенных компонентов.
...
* **Общие братские селекторы** (`E ~ F`)
  Сохраняет «general sibling»‑конструкции, активируемые при динамическом изменении DOM.
...
#### 3. CSS‑функции и переменные

Позволяют оставлять вычисления и динамические свойства.

* **CSS‑переменные** (`--custom-var`)
  Защищает пользовательские свойства, используемые в runtime или через JS.
...
* **Вендорные префиксы** (`-webkit-`, `-moz-`, `-ms-` и др.)
  Сохраняет префиксные свойства для кроссбраузерной поддержки.
...
* **CSS‑функции** (`calc()`, `var()`, `url()`, `min()`, `max()` и др.)
  Оставляет все функциональные выражения, критически важные для динамики макета и ресурсов.
...
#### 4. Эффекты и визуальные свойства

Группа параметров для графических и анимационных эффектов.

* **Анимации и переходы** (`animation`, `transition`)
  Защищает свойства анимаций и связанные ключевые кадры для плавности интерфейса.
...
* **Трансформации** (`transform`)
  Сохраняет 2D/3D‑преобразования для визуальных эффектов и адаптивных изменений.
...
* **Процентные значения** (`50%`, `100%`)
  Оставляет макетные и селекторные правила, зависящие от относительных величин.
...
* **Экранированные символы** (`\00a0`, `\#id` и др.)
  Сохраняет специальные конструкции для compatibility‑селекторов.
...
* **Цветовые функции** (`rgb()`, `rgba()`, `hsl()`, `hsla()`)
  Защищает цветовые определения и прозрачность.
...
* **Градиенты** (`linear-gradient`, `radial-gradient`)
  Оставляет фоны‑градиенты и маски, часто используемые как изображения.
...
* **Фильтры** (`filter`, `backdrop-filter`)
  Сохраняет CSS‑фильтры для размытия, контраста и других визуальных эффектов.
...
* **Маски и обрезка** (`mask`, `clip-path`)
  Оставляет правила для сложных декоративных и функциональных масок.
...
1. **Критические (нельзя убрать)**
   Ат‑правила и ключевые селекторы: `media`, `keyframes`, `pseudo_classes`, `pseudo_elements`, `attribute_selectors`.

2. **Важные (рекомендовано включать)**
   `css_variables`, `css_functions`, `animations`, `vendor_prefixes`, `logical_selectors`.

3. **Дополнительные (можно опционально отключать)**
...
#### Белый список селекторов

Поле для перечисления через запятую CSS‑правил, которые **никогда не удаляются**, даже если они не задействованы на страницах во время проверки. Используйте для защиты динамических или редко встречающихся элементов (модальные окна, всплывающие подсказки, скрытые вкладки и т. п.).

* **Wildcard‑символы**:

  * `*` — 0 или более любых символов

    > Пример: `btn-*` защитит `.btn-primary`, `.btn-lg`, `.btn-xyz`
  * `?` — ровно один любой символ

    > Пример: `icon-?` защитит `.icon-a`, `.icon-1`, но не `.icon-10`

#### Чёрный список селекторов
...
* Пример:
  ...
  * `legacy-*` — удалит всё, что начинается с `.legacy-`
  * `debug-?` — удалит `.debug-a`, `.debug-1` и т. п.

---

> **Обработка любых CSS‑блоков**
> Белый и чёрный списки распространяются не только на обычные селекторы, но и на конструкции типа `@keyframes`, `@font-face`, `@counter-style` и другие «@»‑блоки.

**Порядок приоритизации**

1. Чёрный список
2. Белый список
3. Критические настройки (`@media`, `@supports`, `@keyframes`, `@font-face` и т. д.)
4. Режим генерации
...
* **Убираем все неиспользуемые (remove_unused)**
  ...
  * При этом автоматическая проверка не затрагивает приоритеты белого списка и критических `@`‑блоков — они уже включены в категории used или unknown.
...
* **Убираем всё кроме используемых (keep_used)**
  ...
  * Незадействованные или не проверенные селекторы и `@`‑блоки в этот режим не попадут.
...
1. Скачайте файлы `remove-unused-css.php`, `remove-unused-css.js`, `composer.json`
...
INCLUDE_IMPORT_FILE_COMBINED = true; // включать @import файлы в объединённый файл
```
