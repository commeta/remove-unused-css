...
* Все селекторы хранятся между запусками в `data/selectors.json`.
...
* **Результат:**
  ...
  * JSON-файл `data/selectors.json` пополняется отмеченными селекторами;
...
![Restore files](https://raw.githubusercontent.com/commeta/remove-unused-css/refs/heads/master/backup.png "Restore files")
...
Для ограничения доступа в подкаталоге можно воспользоваться средствами PHP

```php
if ($_SERVER['REMOTE_ADDR'] == '123.123.123.123') {
    return '<script src="/remove-unused-css/remove-unused-css.js"></script>';
}
```
...
* Служить основой для последующей обработки: 
  * В режиме `Убираем все неиспользуемые` при генерации очищенных CSS удаляются только те селекторы, которые в JSON имеют статус `unused`, при этом учитываются исключения по настройкам.
  * В режиме `Убираем всё кроме используемых` при генерации очищенных CSS остаются только те селекторы, которые в JSON имеют статус `used`, при этом учитываются исключения по настройкам.

#### Принцип работы

* При каждом обходе страницы (клиентский скрипт) собирает применяемые селекторы и отправляет на сервер обновлённые данные.
* На сервере происходит объединение: для каждого файла в `selectors.json` отмечаются селекторы, которые встретились (меняют статус на `used`), а отсутствующие после прежних запусках остаются или помечаются как `unused` или `unknown`.
...
* **autoSave**: автоматически сохранять обнаруженные селекторы в `selectors.json` после завершения обхода.
...
* Регулярно проверяйте и резервируйте `selectors.json` и `settings.json`, чтобы иметь возможность восстановить состояние.
