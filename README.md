...
* **BACKUP\_DIR**: папка для резервных копий оригинальных CSS перед обновлением.
...
* При запросе генерации сервер читает `SELECTORS_FILE`, парсит CSS через Sabberworm, удаляет «мертвые» селекторы, затем:
  ...
  1. Парсит AST и удаляет правила по данным JSON.
  2. Если `FIX_PATHS_INDIVIDUAL=true`, корректирует относительные пути внутри каждого файла (например, если файл перемещается).
  3. Если `CSS_MINIFY=true`, выполняет минимизацию (удаление пробелов, комментариев).
  4. Сохраняет результат в `CSS_DIR` с сохранением структуры папок.
